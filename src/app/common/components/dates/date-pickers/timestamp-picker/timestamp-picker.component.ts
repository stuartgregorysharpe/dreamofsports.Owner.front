...
export class CTimestampPickerComponent extends CDatePicker<Date> implements OnChanges {    
    ...
    get formattedDate(): string {
        if (this.value) {
            ...
            return this.beauty ? 
                (this.useTime ? 
                    `${day} ${this.thelang.words["date-monthr-"+month]} ${year} ${hour}:${this.appService.twoDigits(minute)}` : 
                    `${day} ${this.thelang.words["date-monthr-"+month]} ${year}`) : 
                (this.useTime ? 
        ...
        return this.thelang.words["common-empty2"];
    }
